...
public class Rational {
    ...
    //constructor set both numerator and denominator
    Rational(int a, int b){
        if(b == 0) throw new RationalException("denominator cannot be 0");
        int gcd=gcd(a,b);
        numerator = a/gcd;
        denominator = b/gcd;
        
    }
    //constructor set only numerator
    Rational(int a){
        numerator = a;
        denominator = 1;
    }
    //constructor presents the rational to 0
    Rational(){
        numerator = 0;
        denominator = 1;
    }
    ...
    void copy(Rational obj){
        this.numerator = obj.numerator;
        this.denominator = obj.denominator;
    }
    public int getNumerator(){return this.numerator;}  //get numerator
    public int getDenominator(){return this.denominator;}   //get denominator
    //reciprocal
    ...
    //compare 2 rational objs
    public int compareTo(Rational obj){
        if(this.denominator == obj.denominator){
            return this.numerator>obj.numerator? 1 : this.numerator < obj.numerator? -1 :0;
        }else{
            int lcm = lcm(obj);  
            int n1 = lcm/this.denominator*this.numerator;  //numer for this
            int n2 = lcm/obj.denominator*obj.numerator;   //numer for obj
            return n1>n2? 1 : n1<n2? -1 :0;
        }
    }
    //least common multiple
    public int lcm(Rational obj){
        return this.denominator/gcd(this.denominator,obj.denominator)*obj.denominator;
    }
    //equals
    ...
    public Rational add(Rational obj){
        int totalNumer;
        if(this.denominator == obj.denominator){
            totalNumer = this.numerator+obj.numerator;
            return new Rational(totalNumer,this.denominator);
        }else{
            int lcm = lcm(obj);  
            int n1 = lcm/this.denominator*this.numerator;  //numer for this
            int n2 = lcm/obj.denominator*obj.numerator;   //numer for obj
            totalNumer = n1+n2;
            return new Rational(totalNumer, lcm);
        }
    }
    //subtraction
    ...
    public Rational div(Rational obj){
        Rational a = new Rational(obj).inverse();
        return mul(a);
    }

    //add in place
    public Rational addInPlace(Rational obj){
        Rational result = add(obj);
        copy(result); //copy constructor
        return this;
    }
    //sub in place
    public Rational subInPlace(Rational obj){
        Rational result = sub(obj);
        copy(result);  //copy constructor
        return this;
    }
    //mul in place
    public Rational mulInPlace(Rational obj){
        Rational result = mul(obj);
        copy(result);  //copy constructor
        return this;
    }
    //div in place
    public Rational divInPlace(Rational obj){
        Rational result = div(obj);
        copy(result);  //copy constructor
        return this;
    }
